    ...
    <div className={['flex flex-wrap gap-1.5', className].filter(Boolean).join(' ')}>
      {badges.map((b) => {
        const pill = (
          <span
            className={[
              'inline-flex items-center rounded-full px-2 py-0.5 text-[11px] leading-4 font-medium ring-1 ring-inset',
        ...
        return b.href ? (
          <Link key={b.key} href={b.href} className="no-underline hover:opacity-90">
            {pill}
          </Link>
        ) : (
